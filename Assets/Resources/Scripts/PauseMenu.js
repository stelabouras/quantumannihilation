...
function PauseMenu() {
	BeginPage(400,200);
	if (GUILayout.Button (IsBeginning() ? "Play" : "Resume", buttonStyle)) 
	{
		UnPauseGame();
	}
...
//	{
	...
	if(!IsBrowser() && GUILayout.Button("Exit Game", buttonStyle))
	{
		Application.Quit();
	}
	EndPage();
...
function PauseGame() {
	...
	var script			= player.GetComponent(MouseLook);
	script.enabled 		= false;
	var script2	 		= gameObject.GetComponent(MouseLook);
